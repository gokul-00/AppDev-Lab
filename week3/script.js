...
const displayQuest = () => {
    document.getElementById(`quest${cur_quest - 1}`).classList.toggle("d-none");
};

window.addEventListener("load", (event) => {
    console.log("page is fully loaded");

    const parentForm = document.getElementById("questions");
    quiz.forEach((ele, i) => {
        const childEle = document.createElement("div");

        let text = `
    <div id="quest${i}" class="d-none">
    <p>Question <span>${i + 1}</span></p>
    <p id="quest">${quiz[i].question}</p>
    `;
        for (let item of Object.values(quiz[i].options[0])) {
            text += `    <div class="form-check">
        <input class="form-check-input" type="radio" name="q${i}" value="${item}">
        ...
        text += `</div>`;
        childEle.innerHTML = text;
        parentForm.appendChild(childEle);
    });
...
document.getElementById("next").addEventListener("click", (e) => {
    ...
    const radio = document
        .getElementById(`quest${cur_quest - 1}`)
        .querySelector(`input[type=radio][name=q${cur_quest - 1}]`);
    if (radio.checked) {
        document
            .getElementsByClassName(`box`)
            [cur_quest - 1].classList.add("answered");
        document
            .getElementsByClassName(`box`)
            [cur_quest - 1].classList.remove("unanswered");
    }
    if (cur_quest == 25)
...
document.getElementById("clear").addEventListener("click", (e) => {
    ...
    const radio = document
        .getElementById(`quest${cur_quest - 1}`)
        .querySelector(`input[type=radio][name=q${cur_quest - 1}]:checked`);
...
document.getElementById("submit").addEventListener("click", function (e) {
    e.preventDefault();
    console.log(
        document.getElementById("quest0").querySelector("input").checked
    );
});
